...
- **Pedido/Order**: `OrderItem`/`OrderCreateItemPayload`/`OrderUpdateItemDraft` migrados de `product_uuid` único pra `ticket_type_uuid`/`event_product_uuid` (exatamente um por item, mesma regra do backend). `OrderFormPage`/`OrderDetailDialog` perderam a UI de opcionais (sem equivalente); autocomplete de item agora busca `TicketType`+`EventProduct` combinados.
...
- **Pendência sinalizada, não implementada nesta rodada**: tela de detalhe de evento é funcional mas simples (sem galeria de fotos, sem mapa de assento/mesa — aguarda o domínio Venue/Seat da spec); reorder do Portal (`PortalReorderItem`) usa campo `ticket_type_uuid` também para `EventProduct` porque o backend (`PortalCustomerService::getOrderItemsForReorder`) não distingue os dois nesse endpoint especificamente.
