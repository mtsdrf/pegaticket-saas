...
> **Nota de revisão — 7 de agosto de 2026:** checkout e pós-compra do storefront receberam mais uma rodada de fechamento funcional e visual, conferida diretamente no código do `web/` e da trilha PagBank no `api/`:
>
> - **Rastreio público renomeado para compra pública**: a rota principal de acompanhamento do comprador passou a ser `/compra/:uuid`; compatibilidade com a rota antiga foi mantida apenas como ponte técnica.
> - **Checkout sem verificação prévia de e-mail por código**: o e-mail agora é obrigatório no formulário principal do comprador, abaixo do telefone, sem etapa extra de OTP antes da compra.
> - **Telefone e documentos no checkout refinados**: telefone com máscara brasileira válida (fixo/celular com DDD) e CPF/CNPJ com máscara dinâmica conforme digitação nos formulários de cartão.
> - **Cartão PagBank amadurecido no frontend**: validação de número com Luhn, detecção de bandeira, CVV por bandeira, preview visual do cartão, rotação frente/verso ao focar CVV e envio do `expYear` ao PagBank sempre com 4 dígitos.
> - **Fluxo de aprovação da venda simplificado**: compra paga e aprovada no PagBank segue aprovação automática no sistema; compra recusada deixa de cair como "aguardando aprovação" e passa a ficar localmente como não aprovada/rejeitada para o comprador refazer a tentativa.
> - **Pós-compra do comprador ampliado**: ingresso enviado por e-mail após pagamento aprovado, com opção de baixar PDF do ingresso em layout alinhado ao padrão visual transacional.
> - **Convites pendentes deixaram de bloquear reenvio**: o mesmo e-mail pode ser convidado novamente pela empresa quando necessário, sem travar a operação.
...
- **Storefront público**: catálogo de eventos, detalhe do evento, favoritos, carrinho, hold de inventário, checkout e acompanhamento público da compra em `/compra/:uuid`.
...
- **Pagamentos base** *(atualizado 2026-08-07)*: Mercado Pago, Pix e **cartão de crédito/débito via PagBank** (tokenização `card.encrypted`, `createCardCharge()`, `CREDIT_CARD`/`DEBIT_CARD`, parcelamento via `installments`, validação de titular/CPF-CNPJ, máscara dinâmica, Luhn, detecção de bandeira, preview visual do cartão, CVV contextual e `expYear` normalizado com 4 dígitos — interface própria, sem checkout transparente de terceiro, decisão confirmada com o usuário), webhooks, reconciliação e camada de payment issues.
...
- **Portal do comprador**: login OTP, perfil, favoritos, vouchers, minhas vendas; o checkout público não exige mais verificação prévia de e-mail por código antes da compra.
...
- **Checkout** *(atualizado 2026-08-07)*: existe fluxo público consistente, com hold ativo, e-mail obrigatório, telefone mascarado, aprovação automática quando o PSP aprova e rejeição imediata quando o PagBank recusa; ainda faltam formulários por ingresso, retentativas mais maduras e jornada completa de alta demanda.
