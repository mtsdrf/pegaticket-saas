...
class ReportTest extends TestCase
{
    ...
    // --- Listagem de clientes filtrada ---

    #[Test]
    public function user_without_permission_cannot_list_report_clients(): void
    {
        $this->auth()->getJson('/api/v1/reports/clients')->assertStatus(403);
    }

    #[Test]
    public function user_without_permission_cannot_list_report_receivables(): void
    {
        $this->auth()->getJson('/api/v1/reports/receivables')->assertStatus(403);
    }

    #[Test]
    public function user_without_permission_cannot_view_receivables_summary(): void
    {
        $this->auth()->getJson('/api/v1/reports/receivables/summary')->assertStatus(403);
    }

    #[Test]
    public function user_without_update_permission_cannot_store_receivable_interaction(): void
    {
        $client = $this->createClientWithCity($this->tenant->id, 'Campinas');
        $location = $this->createLocation($this->tenant->id);
        $order = $this->createOrder($this->tenant->id, $client, $location, [
            'is_paid' => false,
            'due_date' => now()->subDays(3),
        ]);

        $this->auth()->postJson("/api/v1/reports/receivables/{$order->uuid}/interactions", [
            'interaction_type' => 'note',
            'notes' => 'Tentativa de contato',
        ])->assertStatus(403);
    }

    #[Test]
    public function report_clients_list_includes_only_clients_fully_paid_and_delivered(): void
    {
        $this->grantPermission('reports', 'read');
        $location = $this->createLocation($this->tenant->id);

        // Cliente adimplente: único pedido ativo, pago e entregue — deve aparecer.
        $goodStanding = $this->createClientWithCity($this->tenant->id);
        $this->createOrder($this->tenant->id, $goodStanding, $location, [
            'is_paid' => true, 'paid_at' => now(), 'is_delivered' => true, 'delivered_at' => now(), 'total_amount' => 70,
        ]);

        // Cliente com pendência: 1 pedido quitado + 1 pago mas não entregue —
        // não pode aparecer como "em dia" mesmo tendo um pedido quitado.
        $withPendingOrder = $this->createClientWithCity($this->tenant->id);
        $this->createOrder($this->tenant->id, $withPendingOrder, $location, [
            'is_paid' => true, 'paid_at' => now(), 'is_delivered' => true, 'delivered_at' => now(), 'total_amount' => 70,
        ]);
        $this->createOrder($this->tenant->id, $withPendingOrder, $location, [
            'is_paid' => true, 'paid_at' => now(), 'is_delivered' => false, 'total_amount' => 30,
        ]);

        // Cliente sem nenhum pedido ativo (nunca comprou) — não conta como
        // adimplente por vacuidade, fica de fora do relatório.
        $neverBought = $this->createClientWithCity($this->tenant->id);

        // Cliente cujo único pedido foi cancelado — sem pedido ativo, mesma
        // lógica do "nunca comprou".
        $onlyCancelled = $this->createClientWithCity($this->tenant->id);
        $this->createOrder($this->tenant->id, $onlyCancelled, $location, [
            'cancelled_at' => now(), 'cancellation_reason' => 'Cliente desistiu',
        ]);

        $response = $this->auth()->getJson('/api/v1/reports/clients')->assertStatus(200);

        $response->assertJsonCount(1, 'data');
        $response->assertJsonPath('data.0.uuid', $goodStanding->uuid);
        $response->assertJsonPath('data.0.orders_count', 1);
    }

    #[Test]
    public function report_receivables_list_merges_orders_and_installments_and_supports_filters(): void
    {
        $this->grantPermission('reports', 'read');
        $location = $this->createLocation($this->tenant->id);
        $clientA = $this->createClientWithCity($this->tenant->id, 'Campinas');
        $clientB = $this->createClientWithCity($this->tenant->id, 'Santos');
        Client::whereKey($clientA->id)->update(['phone_primary' => '11999990001']);
        Client::whereKey($clientB->id)->update(['phone_primary' => '11999990002']);

        $this->createOrder($this->tenant->id, $clientA, $location, [
            'is_installment' => false,
            'is_paid' => false,
            'total_amount' => 80,
        ...
        $installmentOrder = $this->createOrder($this->tenant->id, $clientB, $location, [
            'is_installment' => true,
            'is_paid' => false,
            'total_amount' => 120,
        ]);
        ...
        $response = $this->auth()->getJson('/api/v1/reports/receivables')->assertStatus(200);

        $response->assertJsonCount(3, 'data');
        $response->assertJsonPath('data.0.source', 'order');
        $response->assertJsonPath('data.0.client_name', $clientA->name);
        $response->assertJsonPath('data.0.client_phone_primary', '11999990001');
        $response->assertJsonPath('data.0.amount', 80);
        $response->assertJsonPath('data.0.is_overdue', true);

        $overdueOnly = $this->auth()->getJson('/api/v1/reports/receivables?is_overdue=1')->assertStatus(200);
        $overdueOnly->assertJsonCount(2, 'data');

        $clientFiltered = $this->auth()->getJson('/api/v1/reports/receivables?client_name=' . urlencode($clientB->name))->assertStatus(200);
        $clientFiltered->assertJsonCount(2, 'data');
        $clientFiltered->assertJsonPath('data.0.client_name', $clientB->name);

        $bucketFiltered = $this->auth()->getJson('/api/v1/reports/receivables?aging_bucket=overdue_1_30')->assertStatus(200);
        $bucketFiltered->assertJsonCount(2, 'data');
    }

    #[Test]
    public function receivables_summary_returns_open_and_overdue_totals(): void
    {
        $this->grantPermission('reports', 'read');
        $location = $this->createLocation($this->tenant->id);
        $client = $this->createClientWithCity($this->tenant->id, 'Campinas');
        ...
        $response = $this->auth()->getJson('/api/v1/reports/receivables/summary')->assertStatus(200);

        $response->assertJsonPath('data.open_amount', '200.00')
            ->assertJsonPath('data.open_count', 3)
            ->assertJsonPath('data.overdue_amount', '140.00')
            ->assertJsonPath('data.overdue_count', 2)
            ->assertJsonPath('data.aging_buckets.0.bucket', 'current')
            ->assertJsonPath('data.aging_buckets.1.bucket', 'overdue_1_30');
    }

    #[Test]
    public function receivable_interaction_can_be_created_and_listed_for_installment(): void
    {
        $this->grantPermission('reports', 'read');
        $this->grantPermission('reports', 'update');
        $location = $this->createLocation($this->tenant->id);
        $client = $this->createClientWithCity($this->tenant->id, 'Campinas');
        $order = $this->createOrder($this->tenant->id, $client, $location, [
            'is_installment' => true,
        ...
        $create = $this->auth()->postJson("/api/v1/reports/receivables/{$order->uuid}/interactions", [
            'installment_uuid' => $installment->uuid,
            'interaction_type' => 'promise',
            'notes' => 'Cliente prometeu pagar na sexta',
            'promised_amount' => 60,
            'promised_due_date' => now()->addDays(2)->toDateString(),
        ])->assertStatus(201);

        $create->assertJsonPath('data.interaction_type', 'promise')
            ->assertJsonPath('data.installment_uuid', $installment->uuid);

        $list = $this->auth()->getJson("/api/v1/reports/receivables/{$order->uuid}/interactions?installment_uuid={$installment->uuid}")
            ->assertStatus(200);

        $list->assertJsonCount(1, 'data');
        $list->assertJsonPath('data.0.interaction_type', 'promise');
        $list->assertJsonPath('data.0.notes', 'Cliente prometeu pagar na sexta');
    }
    ...
    #[Test]
    public function clients_pdf_endpoint_returns_a_valid_pdf_download(): void
    {
        $this->grantPermission('reports', 'export_pdf');
        $client = $this->createClientWithCity($this->tenant->id);
        $location = $this->createLocation($this->tenant->id);
        $this->createOrder($this->tenant->id, $client, $location, [
            'is_paid' => true, 'paid_at' => now(), 'is_delivered' => true, 'delivered_at' => now(),
        ]);

        $response = $this->auth()->postJson('/api/v1/reports/clients/pdf');
