...
class OrderCancellationApprovalTest extends TestCase
{
     ...
     */
    private function createOrderWithCancellationRequested(): Order
    {
    ...
    public function staff_approves_cancellation_request_and_executes_the_cancellation(): void
    {
        ...
        $response = $this->auth()->postJson('/api/v1/orders/' . $order->uuid . '/approve-cancellation');
    ...
    public function staff_rejects_cancellation_request_and_reverts_status(): void
    {
        ...
        $response = $this->auth()->postJson('/api/v1/orders/' . $order->uuid . '/reject-cancellation');
    ...
    public function approving_cancellation_fails_when_no_request_is_open(): void
    {
        ...
        $this->auth()->postJson('/api/v1/orders/' . $order->uuid . '/approve-cancellation')
            ->assertStatus(422)
            ->assertJsonPath('code', 'INVALID_ORDER_STATE');

        $this->auth()->postJson('/api/v1/orders/' . $order->uuid . '/reject-cancellation')
            ->assertStatus(422)
            ->assertJsonPath('code', 'INVALID_ORDER_STATE');
    }
}
