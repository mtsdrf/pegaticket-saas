 ...
 */
class SaleInstallmentTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function cannot_create_installment_for_non_installment_order(): void
    {
        ...
        $this->auth()->postJson("/api/v1/sales/{$order['uuid']}/installments", [
    ...
    {
        $this->grantPermission('sales', 'update');
        $this->grantPermission('sales', 'delete');
        ...
        $this->auth()->putJson("/api/v1/sales/{$order['uuid']}/installments/{$installments[0]['uuid']}", [
            'installment_number' => 1,
            'amount' => 50,
            'due_date' => '2026-08-10',
        ])->assertStatus(422)->assertJsonPath('code', 'INVALID_ORDER_STATE');

        $this->auth()->deleteJson("/api/v1/sales/{$order['uuid']}/installments/{$installments[0]['uuid']}")
            ->assertStatus(422)->assertJsonPath('code', 'INVALID_ORDER_STATE');
    }
    ...
    public function create_breaking_the_total_sum_is_rejected(): void
    {
        ...
        $response = $this->auth()->postJson("/api/v1/sales/{$order['uuid']}/installments", [
            'installment_number' => 3,
            'amount' => 10,
            'due_date' => '2026-08-10',
        ]);
    ...
    public function update_breaking_the_total_sum_is_rejected(): void
    {
        ...
        $response = $this->auth()->putJson("/api/v1/sales/{$order['uuid']}/installments/{$installments[0]['uuid']}", [
            'installment_number' => 1,
    ...
    public function delete_breaking_the_total_sum_is_rejected(): void
    {
        ...
        $response = $this->auth()->deleteJson("/api/v1/sales/{$order['uuid']}/installments/{$installments[0]['uuid']}");
    ...
    public function creating_a_new_installment_succeeds_when_sum_still_matches_total(): void
    {
        ...
        SaleInstallment::where('uuid', $installments[1]['uuid'])->update(['amount' => 30.00]);
    ...
    public function deleting_and_recreating_an_installment_succeeds_when_sum_still_matches_total(): void
    {
        ...
        SaleInstallment::where('uuid', $installments[0]['uuid'])->update(['amount' => 100.00]);

        $this->auth()->deleteJson("/api/v1/sales/{$order['uuid']}/installments/{$installments[1]['uuid']}")
            ->assertStatus(204);
        ...
        SaleInstallment::where('uuid', $installments[0]['uuid'])->update(['amount' => 60.00]);
    ...
    public function reallocate_redistributes_three_installments_into_two(): void
    {
        ...
        $response = $this->auth()->putJson("/api/v1/sales/{$order['uuid']}/installments", [
            'installments' => [
    ...
    public function reallocate_can_add_a_new_installment_while_keeping_the_sum(): void
    {
        ...
        $this->assertEquals(3, SaleInstallment::whereNull('deleted_at')->count());
    }
    ...
    public function reallocate_is_blocked_for_cancelled_order(): void
    {
        ...
        $this->auth()->putJson("/api/v1/sales/{$order['uuid']}/installments", [
            'installments' => [
     ...
     * exclusão/criação/edição real (ver SaleInstallmentService::reallocate).
     */
    #[Test]
    ...
    public function individual_delete_then_create_can_reuse_the_same_installment_number(): void
    {
        ...
        SaleInstallment::where('uuid', $installments[0]['uuid'])->update(['amount' => 50.00]);
