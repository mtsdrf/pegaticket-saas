...
class CorsTest extends TestCase
{
    ...
    {
        config()->set('cors.allowed_origins', ['https://app.pegaticket.test']);

        $response = $this
            ->withHeader('Origin', 'https://app.pegaticket.test')
            ->withHeader('Access-Control-Request-Method', 'POST')
            ->withHeader('Access-Control-Request-Headers', 'Authorization, Content-Type')
            ->call('OPTIONS', '/api/v1/auth/login');

        $response->assertSuccessful();
        $response->assertHeader('Access-Control-Allow-Origin', 'https://app.pegaticket.test');
    }
}
