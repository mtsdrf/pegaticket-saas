...
class OrderPaymentService
{
     ...
     */
    public function createPixChargeForOrder(Order $order): Payment
    {
        ...
        DB::transaction(function () use ($order) {
            ...
            if ($this->activePendingCharge($locked) !== null) {
                throw new InvalidOrderStateException(__('messages.order.payment_charge_already_active'));
            }
        });
     ...
     * criar uma segunda.
     */
    public function activePendingCharge(Order $order): ?Payment
    {
     ...
     */
    public function paidChargeForOrder(Order $order): ?Payment
    {
        return Payment::query()
            ->whereNull('deleted_at')
            ->where('payable_type', $order->getMorphClass())
     ...
     * caso o cancelamento segue seu guard normal em OrderService).
     */
    public function createRefundForPaidCancellation(Order $order): ?Refund
    {
        ...
        event(new OrderPaymentRefundRequested(
            orderUuid: $order->uuid,
     ...
     * cuidado de IDOR aplicado em OrderService.
     */
    private function assertBelongsToCurrentTenant(Order $order): void
    {
