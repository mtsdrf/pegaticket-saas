...
- **[ALTA/bug real] `POST /v1/orders` sem `payer`** — confirmado contra a API real (não só `Http::fake`) que a criação de cobrança Pix de pedido falhava sempre por faltar `payer`. Regra nova: toda vez que um payload de PSP exigir dado de "quem paga", resolver pela cadeia de posse real (`Client`→`FinalCustomerTenantLink` CONFIRMADO→`FinalCustomer`), nunca usar o e-mail do TENANT/loja como substituto do cliente final — são payers diferentes. Fallback final aceitável: dado real já cadastrado (nome, CPF/CNPJ), nunca inventado.
