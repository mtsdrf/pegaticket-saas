...
- **Vencimento de parcela**: não existe coluna dedicada, mas a regra existe na aplicação — ao criar um `venda` não pago, `data_pagamento` é preenchida com **dia 10 do mês seguinte** (`Venda/CadastrarController`), funcionando como "data de vencimento esperada" até o pagamento real substituir esse valor. Resolve a dúvida nº4 anterior.
- **`venda.entregue` default 1 no banco é irrelevante na prática**: a aplicação sempre define `entregue` explicitamente a partir do request tanto no create quanto no update — o default do banco nunca é exercitado pelo fluxo normal. Não precisa ser replicado no novo modelo.
- **Venda não pode ser excluído**: a rota `DELETE venda/{venda}` está **comentada** em `routes/api.php` — exclusão de venda foi deliberadamente desabilitada no legado. Só existem ações de update (`alterar`, `alterarEntregue`, `alterarPago`).
- **Cascata de quitação por parcela**: ao cadastrar uma nova `venda_parcela`, se **todas** as parcelas daquele venda já estiverem pagas (incluindo a nova), o `venda` inteiro é automaticamente marcado `entregue=true, pago=true, valor_pago=valor_total` (`VendaParcela/CadastrarController`). Regra de negócio real, não óbvia pelo schema.
- **Saldo devedor do cliente**: calculado sob demanda (não hay coluna), somando por cliente: se `parcelado`, `valor_total - soma(valor_pago das parcelas)`; senão, `valor_total - valor_pago` do próprio venda (`Cliente/PesquisarController::vendas`/`clienteVenda`).
...
- **Indicadores de dashboard confirmados** (`Relatorio/PesquisarController::pesquisarIndicadores`, usados também no `Home/PesquisarController` para a tela inicial): total de vendas, vendas entregues/não entregues, vendas pagos/não pagos, **valor_recebido** (soma `valor_pago` dos pagos) e **valor_a_receber** (soma `valor_total` dos não pagos) — com filtro opcional de período. Bate exatamente com os 3 cards que já estavam no placeholder do `DashboardPage.tsx` (Vendas entregues / Vendas pendentes / Valor recebido) — confirma que a direção estava certa, e revela 2 métricas a mais (pagos/não pagos, a receber).
- **Gráficos confirmados** (`Relatorio/PesquisarController::pesquisarGraficos`): vendas por mês (linha/barra), vendas pagos×não pagos (pizza), vendas entregues×não entregues (pizza), valor recebido×a receber (pizza), **vendas por cidade** (barra, via join venda→cliente→endereco→cidade). Todos calculados sob demanda via query, nenhum vem de view.
- **Relatório em PDF é um módulo real**, não estava no plano anterior: `Relatorio/PesquisarController::gerarRelatorioPdf` gera PDF (via `barryvdh/laravel-dompdf`) de dois tipos — "vendas" e "clientes" — a partir de uma view Blade (`relatorio_venda_pdf`, `relatorio_cliente_pdf`).
...
- `venda_produto.valor_momento_venda` é snapshot independente de `produto.valor`.
- `venda_produto.quantidade_produto` é `float` (permite fração).
...
7. **Paginação obrigatória** em Cliente (e provavelmente Produto/Venda) desde o primeiro CRUD novo — o legado já opera com ~2000 clientes sem paginar, o novo não deve repetir isso.
