...
| `json` | direto | 1 | 3 |
| `venda` | direto | **38.052** | 38.172 |
| `venda_produto` | indireto via `venda_id → venda.estabelecimento_id` | **92.246** | 92.592 |
| `venda_parcela` | indireto via `venda_id → venda.estabelecimento_id` | **0** | 8 |
...
## 2. `venda` — schema completo e distribuição real dos dados (estab 4)
...
**Distribuição real (38.052 vendas do estab 4)**:
- `ativo`: sempre `1` (nunca `0`) — não há "soft-delete por flag" exercido para venda deste tenant, apesar do campo existir.
- `entregue`: `1` em 38.042, `0` em 10.
- `pago`: `1` em 35.857, `0` em 2.195.
- `parcelado`: sempre `0` — **nenhum venda do estabelecimento 4 usa parcelamento**, apesar do campo existir e de haver linhas de `venda_parcela` no banco (todas pertencem ao estab 3, ver seção 3).
...
- `valor_total`: soma de **R$ 4.217.946,33** nos 38.052 vendas.
...
## 3. `venda_produto` — schema e integridade (estab 4)
...
-- FK venda_id→venda, produto_id→produto (RESTRICT implícito)
```

- `valor_momento_venda` é **congelado no momento da venda** (não é FK pro preço atual do `produto` — é uma cópia numérica). **CORREÇÃO (2026-07-22, ver seção 5)**: a frase original aqui dizia "equivalente ao `sale_items.unit_price` atual" — ERRADA. Confirmado por reconciliação 100% contra os 38.052 vendas do estab4: `valor_momento_venda` já é o TOTAL da linha (quantidade-inclusive), equivalente a `sale_items.line_total`, não a `unit_price`. `unit_price` correto = `valor_momento_venda / quantidade_produto`.
- Não existe campo de desconto por item — qualquer desconto só aparece implícito na diferença entre `venda.valor_total` e a soma dos itens (ver seção 5).
- 0 linhas órfãs (`venda_id` inexistente) e 0 linhas com `produto_id` inexistente em toda a tabela (não só no escopo do estab 4) — integridade referencial 100% íntegra nesse ponto, apesar de não haver `FOREIGN KEY` real impedindo isso no schema... **correção**: há sim `ADD CONSTRAINT fk_venda_produtoXvenda`/`fk_venda_produtoXproduto` reais no dump (ver seção 7), então a integridade é garantida pelo próprio banco, confirmada também pelos dados.

## 4. `venda_parcela` — schema e achado crítico de volume
...
-- FK venda_id→venda (RESTRICT)
```

**Achado**: a tabela inteira (todos os 4 estabelecimentos) tem só **8 linhas** no dump (`AUTO_INCREMENT=14`, ou seja só 13 linhas já existiram historicamente, 5 já removidas fisicamente — não há `ativo=0` nelas, foram de fato `DELETE`d, o que contradiz o padrão "nunca apaga fisicamente" observado em [[01-schema-overview]] para as outras tabelas). Das 8 linhas restantes, **todas pertencem a `venda_id` do estabelecimento 3** — nenhuma ao estabelecimento 4. Confirmado também por: nenhum `venda` do estab 4 tem `parcelado=1` (seção 2).

Conclusão prática: `venda_parcela` é **irrelevante para a migração do estabelecimento 4** — não há nenhuma linha para migrar. O equivalente atual (`sale_installments`) não precisa ser populado para este tenant.

## 5. Integridade cruzada `venda` × `venda_produto` (estab 4) — achado mais relevante

**CORRIGIDO INTEIRAMENTE em 2026-07-22 — a análise original abaixo (mantida riscada/histórica por transparência) partiu de uma coluna interpretada errada.** A comparação certa é `venda.valor_total` contra `SUM(venda_produto.valor_momento_venda)` **sem multiplicar por `quantidade_produto`** (o campo já é o total da linha, não o preço unitário — ver seção 3). Refeita essa comparação para os 38.052 vendas do estab 4 (38.049 com pelo menos 1 item, 3 vazios): **38.049 de 38.049 batem exatamente (diferença < R$0,01, 0 divergências)**. Confirmado de novo, de forma independente, pelo dry-run de `legacy:migrate-estabelecimento` (reconciliação item-a-item embutida no comando). Não existe desconto/acréscimo implícito nenhum — `venda.valor_total` é sempre e exatamente a soma dos `valor_momento_venda` dos itens.

**Análise original (ERRADA, mantida para rastreabilidade — não usar)**: ~~Comparando `venda.valor_total` contra `SUM(venda_produto.valor_momento_venda * quantidade_produto)` por venda: 24.078 vendas batiam exato, 13.405 "menor" (desconto implícito suposto), 566 "maior" (acréscimo implícito suposto), 3 vazios. "Achado grave isolado": venda `id=21131` teria `valor_total=32.400,00` mas soma dos itens `R$29.160.000,00` (900 unidades × R$32.400), sugerindo erro de digitação de quantidade, com padrão semelhante em mais 741-4.157 vendas.~~ **Nenhuma dessas divergências era real** — eram todas o mesmo artefato de multiplicar `valor_momento_venda` (já total) por `quantidade_produto` de novo. O venda `id=21131` é um venda válido: 900 unidades × R$36,00/unidade = R$32.400,00 total, batendo exato com `valor_total`. Não havia desconto, não havia acréscimo, não havia erro de digitação — era 100% interpretação errada da coluna, tanto na análise Python original quanto na primeira versão do comando de migração desta sessão.

**3 vendas** (`id` 36195, 36492, 36533) continuam **genuinamente vazios** (zero linhas em `venda_produto`, `valor_total=0.00`) — isso não muda com a correção.

Nenhum órfão de FK encontrado: 0 `venda_produto` com `venda_id` inexistente, 0 com `produto_id` inexistente (em toda a tabela, todos os 4 estabelecimentos).
...
3 linhas no dump inteiro, uma por estabelecimento com dado (`estabelecimento_id` 2, 3 e 4), sempre `tabela='venda'`. É um **snapshot/backup histórico de `venda`** em formato JSON denormalizado (cada item do array tem `id`, `cliente_id`, **`nome`** do cliente embutido, `produtos`/`produto_ids`, valores em formato string BR "161,00", datas em `dd/mm/aaaa`) — aparentemente gerado numa migração/reestruturação anterior do próprio sistema legado (formato antigo do venda, antes de virar a estrutura relacional atual com `venda_produto`).

Para o estabelecimento 4: 3.951 entradas, `id` de venda variando entre 9 e 4067. **Todos os 3.951 ids já existem na tabela `venda` atual** (0 ids exclusivos do snapshot) — ou seja, não há dado de venda "perdido" só recuperável via essa tabela. **Conclusão: `json` pode ser ignorada na migração** — é histórico redundante, já coberto por `venda`/`venda_produto`. Contém nome de cliente embutido em texto livre (PII) — não reproduzido aqui além da estrutura.

## 7. `cliente` vs `cliente_novo`

`cliente` (1913 linhas do estab 4) é a entidade real, com toda a auditoria/tenant/FK padrão (`estabelecimento_id`, `endereco_id`, `dia_ideal_id`, `periodo_ideal_id` etc.) — é o que popula `venda.cliente_id`.
...
### `venda`/`venda_produto`/`venda_parcela` → `sales`/`sale_items`/`sale_installments` + `payments` (comparação mais importante)
| Legado | Atual | Observação |
|---|---|---|
| `venda.estabelecimento_id` | `sales.tenant_id` | bate |
| `venda.cliente_id` | `sales.client_id` (FK NOT NULL) | bate |
| — | `sales.stock_location_id` (FK **NOT NULL**) | **legado não tem conceito de local de estoque** — sales atual exige 1 `stock_location_id` obrigatório. Precisa criar 1 `stock_location` (provavelmente "Loja"/default) para o tenant antes de migrar vendas — **decisão do usuário** |
| `venda.parcelado` | `sales.is_installment` | bate semanticamente, mas **sempre `false`** para o estab 4 (seção 2/4) — `sale_installments` fica vazio para este tenant |
| `venda.valor_total` | `sales.total_amount` | bate, mas atual é **sempre calculado no backend a partir da soma de `sale_items`** (comentário explícito na migration) — os 13.971 vendas do estab 4 (seção 5) onde `valor_total` ≠ soma dos itens **não podem ser recriados via fluxo normal de criação de venda**; precisam de inserção direta que preserve o `total_amount` histórico, ignorando a regra "calculado" (procedimento de migração, não do fluxo de app) |
| `venda.valor_pago` | `sales.paid_amount` | bate — comentário da migration atual já cita literalmente "paridade com o legado, campo valor_pago", então o mapeamento já foi validado antes por quem escreveu a migration |
| `venda.pago` | `sales.is_paid` | bate |
| `venda.data_pagamento` | `sales.paid_at` | bate |
| `venda.entregue` | `sales.is_delivered` | bate |
| `venda.data_entrega` | `sales.delivered_at` **e/ou** `sales.expected_delivery_date` | **ambíguo** — atual separa "data prevista" (`expected_delivery_date`, imutável) de "data real de entrega" (`delivered_at`, setada só por ações de sistema). Legado tem 1 único campo `data_entrega`. Qual dos dois vira o destino da migração é decisão do usuário |
| — | `sales.due_date` | sem fonte direta no legado (venda não-parcelado não tem vencimento explícito) — fica `NULL`? |
| `venda.observacao` | `sales.notes` | bate |
| **sem equivalente** | `sales.discount_amount` | **poderia** receber a diferença negativa calculada na seção 5 (13.405 vendas), mas isso é reconstrução, não dado direto do legado — decisão do usuário |
| **sem equivalente** | `sales.delivery_fee` | idem, para os 566 vendas com `valor_total` > soma dos itens — mas também pode ser erro de digitação (seção 5), não frete real. **Ambíguo, não decidir sozinho** |
| **sem equivalente** | `sales.coupon_id`, `cashback_redeemed_amount`, `service_fee`, `cash_session_id` | sistema legado não tinha cupom/cashback/comanda/caixa — ficam vazios/NULL, sem perda de informação real |
| **sem equivalente** | `sales.status` (string, default `confirmed`) e `sales.origin` (default `staff`) | legado não tem status textual nem origem — inferir de `pago`+`entregue`? Precisa de tabela de decisão explícita do usuário (o quê vira `cancelled`? o legado não tem conceito de cancelamento — sem `ativo=0` em nenhum venda do estab 4) |
| `venda.ativo` | soft delete atual (`deleted_at`) | nunca `0` nos dados do estab 4 (seção 2) — não há venda "desativado" a preservar como soft-deleted |
| `venda_produto.produto_id`/`venda_id` | `sale_items.product_id`/`sale_id` | bate |
| `venda_produto.quantidade_produto` (float 10,2) | `sale_items.quantity` (**decimal 12,3**) | mesma ideia (quantidade fracionária suportada nos dois lados), mas atual tem 3 casas decimais contra 2 do legado — sem perda, só folga extra |
| `venda_produto.valor_momento_venda` | `sale_items.unit_price` (decimal 10,2) | bate, ambos "preço congelado no momento da venda" (mesmo conceito, comentário idêntico na migration atual) |
| — | `sale_items.line_total` | **calculado** (`unit_price * quantity`), sem equivalente direto no legado — mas dá pra derivar direto na migração, sem ambiguidade |
| `venda_parcela` (0 linhas para estab 4) | `sale_installments` | **nada a migrar** para este tenant (seção 4) |
| **sem equivalente** | `payments` (polimórfica, `payable_type`/`payable_id`→`sales`, campos `method`, `status`, `amount`, `provider`) | legado **não registra forma de pagamento** em nenhuma tabela (nem `venda`, nem `venda_parcela`) — `payments.method` fica sempre `NULL` se migrar 1 `payment` por venda pago, ou a migração pode optar por não popular `payments` nenhuma (só `sales.is_paid`/`paid_amount`). **Decisão do usuário**: criar `payments` sintéticos ou não |
...
1. **Nenhum órfão de FK** em toda a cadeia checada: `venda→cliente`, `cliente→endereco`, `cliente→dia_ideal`/`periodo_ideal`, `produto→tipo_produto`, `tipo_produto→categoria_produto`, `venda_produto→venda`/`produto` — 0 casos em todas as checagens.
...
3. ~~**`venda.valor_total` diverge da soma de `venda_produto`** em 13.971 de 38.052 vendas (36,7%)~~ — **RETIRADO, 2026-07-22**: era artefato de interpretação errada de `valor_momento_venda` como preço unitário (ver seção 5, correção completa). A fórmula certa (`SUM(valor_momento_venda)` sem multiplicar por quantidade) bate exato em 38.049 de 38.049 vendas com item.
4. ~~**1 venda (`id=21131`) com erro grave de quantidade**~~ — **RETIRADO, 2026-07-22**: não é erro de digitação, é venda válido (900un × R$36,00 = R$32.400,00 total, batendo exato com `valor_total`). Não existem "centenas de outros vendas" com esse problema — não havia problema nenhum.
5. **3 vendas "vazios"** (sem nenhuma linha em `venda_produto`), `valor_total=0`.
6. **`venda_parcela`**: 0 linhas para o estab 4 (as 8 linhas existentes no banco inteiro pertencem ao estab 3) — tabela irrelevante para esta migração.
...
8. **`json`**: contém só histórico já coberto por `venda`/`venda_produto` (0 ids exclusivos) — seguro ignorar na migração.

## Decisões pendentes — precisam de consulta ao usuário antes de qualquer plano/script de migração

1. ~~**`valor_total` ≠ soma dos itens (13.971 vendas)**~~ — **RESOLVIDO por correção de interpretação, não por decisão de negócio (2026-07-22)**: não havia divergência real (ver seção 5). `total_amount` migra como `venda.valor_total` verbatim de qualquer forma (decisão de negócio do usuário, mantida), mas não há mais "achado de discrepância" a explicar.
2. ~~**Venda `id=21131` e os ~740+ outros com discrepância grande de quantidade**~~ — **RESOLVIDO, não existiam** (ver seção 5).
3. **3 vendas sem itens** (`id` 36195, 36492, 36533): migrar como venda vazio, ou descartar?
4. **`sales.stock_location_id` é obrigatório e não existe no legado**: criar 1 `stock_location` default para o tenant antes de migrar os 38.052 vendas?
5. **`sales.status`/`origin`**: sem equivalente no legado — qual regra determina `status` (confirmed/cancelled/etc.) a partir de `pago`+`entregue`+ausência de `ativo=0`? `origin` sempre `'staff'`?
6. **`sales.delivered_at` vs `expected_delivery_date`**: `venda.data_entrega` do legado vira qual dos dois?
7. **`payments` (polimórfica)**: criar 1 `payment` sintético por venda pago (sem `method` real, já que o legado não registra forma de pagamento), ou não popular `payments` e manter só `sales.is_paid`/`paid_amount`?
...
**Comando novo**: `php artisan legacy:migrate-estabelecimento {--dump=} {--commit}` (`api/app/Console/Commands/MigrateLegacyEstablishmentCommand.php` + parser dedicado `api/app/Console/Commands/Support/LegacyDumpParser.php`, tokenizer character-a-character do dump, sem depender de banco legado vivo). Sem `--commit`: só relatório, zero escrita — dry-run rodado em 2026-07-22 contra o dump real, TODAS as contagens/somas bateram exatamente com os números já validados independentemente (38.052 vendas, 92.246 itens, 1.913 clientes, 176 produtos, 628 endereços, 48 bairros, 6 cidades, 1 estado, 31 dia_ideal, 2 usuários, `SUM(valor_total)`=R$4.217.946,33, `SUM(valor_pago)`=R$3.226.599,06, 35.857 vendas pagos) — dump íntegro, parser correto. Nenhum `User`/`Tenant` pré-existente colide (e-mails/slug livres no banco de `.env` no momento do teste). `--commit` foi implementado por completo (schema/DTO reais, 1 única `DB::transaction()`, guard de idempotência por slug, reconciliação final por `SUM(sales.total_amount)` com `throw` se não bater) mas **NUNCA EXECUTADO** — só o dry-run rodou.

**Achado RESOLVIDO em 2026-07-22 (mesmo dia, revisão de coordenação)**: o parágrafo acima (removido) apontava a suposição de "`valor_momento_venda` = preço unitário" desta seção como contrariada por um bug documentado em `coding-standards.md` (2026-07-14, `ImportLegacyJsQueijosCommand`). Confirmado por parsing direto do dump contra os 38.052 vendas do estab4: a suposição original ESTAVA ERRADA, o bug documentado é que estava certo — `valor_momento_venda` já é o TOTAL da linha. `MigrateLegacyEstablishmentCommand` foi corrigido: `line_total = valor_momento_venda` (direto), `unit_price = valor_momento_venda / quantidade_produto`. Reconciliação item-a-item embutida no dry-run confirma **38.049 de 38.049 vendas-com-item batendo exato** (0 divergência) contra `sales.total_amount`. Ver seção 5 (reescrita) para o histórico completo do erro e da correção. `sales.total_amount` nunca mudou (sempre `venda.valor_total` verbatim, decisão de negócio inalterada).
...
- `sales`: 38.052 (esperado 38.052) ✓
- `sale_items`: 92.246 (esperado 92.246) ✓
- `payments`: 35.857 (esperado 35.857) ✓
- `clients`: 1.913 ✓, `products`: 176 ✓
- `SUM(sales.total_amount)`: R$4.217.946,33 exato ✓ (reconciliação dentro da própria transação também passou, sem `throw`)
- `sales.codigo`: 0 nulos, 38.052 valores distintos (sem colisão da unique `uniq_tenant_order_codigo`), `tenants.next_sale_code` = 39.051 (999 + 38.052, consistente) — `sales:backfill-codigo` rodou certo dentro da transação
- **Reconciliação item-a-item** (`SUM(sale_items.line_total)` por venda vs `sales.total_amount`): **38.049 de 38.049 vendas-com-item exatos, 0 divergência**, 3 vendas vazios (os mesmos IDs já esperados) — confirma a fórmula corrigida (ver achado acima) na escrita real, não só no dry-run
...
- Jenifer (funcionária): `User` criado, role `funcionario` com EXATAMENTE as 10 permissões pedidas (`sales:read/create/deliver/pay`, `clients:read/create/update`, `products:read`, `dashboard:read`, `reports:read`), nem mais nem menos
- `tenant_settings.block_order_without_stock` = `false` ✓; `stock_locations`: 1 registro "Loja", `is_default=true` ✓

Banco descartável destruído após validação (`rm` do arquivo sqlite). `composer test` rodado de novo depois de descartar: 843/843, ambiente principal (`.env`/`DB_HOST` real) confirmadamente intocado (contagem de `tenants`/`sales` idêntica antes/depois: 3 tenants, 28 sales).
