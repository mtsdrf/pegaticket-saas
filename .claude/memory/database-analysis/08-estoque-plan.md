# Módulo de Estoque — Plano (Fase 4, Parte 1) + integração com Pedido (Fase 5, Parte 2)

Pedido do usuário (2026-07-09): módulo completo de gerenciamento de estoque **antes** do Pedido, e o Pedido já nasce integrado com estoque. Usuário forneceu uma lista de 300 capacidades possíveis de um sistema de estoque, com a instrução explícita: **não precisa ter tudo agora, mas precisa estar tudo no radar** (nada pode ser esquecido/perdido, mesmo o que não entra no MVP).
...
- `source_type`+`source_id` (polimórfico nullable — vai apontar para `Pedido` na Parte 2) — item 50.
...
### 6. Reserva de estoque — mecanismo que a Parte 2 (Pedido) vai consumir
`reserve`/`reserve_cancel` incrementam/decrementam `quantity_reserved` em `StockBalance`. Ao criar um Pedido: reserva o estoque dos itens. Ao marcar entregue: converte reserva em saída real (`exit`). Ao cancelar: `reserve_cancel` (se ainda não convertida) ou `reversal` (se já tinha saído).
...
- **Indicadores avançados** (106-117): abaixo do mínimo/acima do máximo (dá pra calcular via query simples quando pedido, mas sem dashboard dedicado ainda), giro, Curva ABC, sugestão de reposição, previsão de ruptura — nascem junto com a Fase de Relatórios/Dashboard (renumerada para Fase 6).
...
- **Workflow de transferência com estados** (188-201): criada/enviada/recebida/cancelada, conferência, recebimento parcial — MVP é transferência atômica sem estados intermediários. Item 201 ("baixa automática por venda/pedido") **é justamente o core da Parte 2**, não é radar.
