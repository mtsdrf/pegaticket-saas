...
function makeMarketplaceSale(overrides: Partial<MarketplaceSaleMock> = {}): MarketplaceSaleMock {
  return {
...
function buildOperationsSummary(order: MarketplaceSaleMock) {
...
test.describe('Pedidos iFood', () => {
  ...
  test('opera a fila iFood com importação, ações operacionais e cancelamento auditável', async ({ page }) => {
    ...
    const integration = makeIntegration()
    let currentOrder = makeMarketplaceSale()
    let actionCounter = 1
