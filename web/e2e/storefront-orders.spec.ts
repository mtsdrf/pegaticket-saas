...
test.describe('Pedidos da loja', () => {
  test('mostra fila da loja com board e grid sem depender da central manual', async ({ page }) => {
    await mockAuthenticatedShell(page, {
      tenantSelectionConfirmed: true,
      tenantPermissions: ['storefront-orders:read', 'orders:update'],
      tenantFunctionalities: ['storefront-orders'],
    })
    ...
    await expect(page.getByRole('gridcell', { name: 'Cliente Loja QA', exact: true })).toBeVisible()
  ...
  test('move pedidos no board entre etapas e exige motivo ao cancelar', async ({ page }) => {
    ...
    await page.route('**/api/v1/storefront-orders/storefront-order-approval-1/approve', async (route) => {
    ...
    await page.route('**/api/v1/storefront-orders/storefront-order-production-1/cancel', async (route) => {
    ...
    await page.route('**/api/v1/storefront-orders*', async (route) => {
      ...
      if (method === 'GET' && /\/api\/v1\/storefront-orders(\?.*)?$/.test(url)) {
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
    ...
    const approveResponse = page.waitForResponse(
      (response) =>
        response.url().includes('/storefront-orders/storefront-order-approval-1/approve')
    ...
    const cancelResponse = page.waitForResponse(
      (response) =>
        response.url().includes('/storefront-orders/storefront-order-production-1/cancel')
  ...
  test('aprova ou rejeita solicitações de cancelamento com reflexo operacional na fila', async ({ page }) => {
    ...
    await page.route('**/api/v1/orders/storefront-cancel-approve-1/approve-cancellation', async (route) => {
      if (route.request().method() !== 'POST') {
        await route.fallback()
        return
      }
    ...
    await page.route('**/api/v1/orders/storefront-cancel-reject-1/reject-cancellation', async (route) => {
      if (route.request().method() !== 'POST') {
        await route.fallback()
        return
      }
    ...
    await page.route('**/api/v1/storefront-orders/storefront-cancel-approve-1*', async (route) => {
    ...
    await page.route('**/api/v1/storefront-orders/storefront-cancel-reject-1*', async (route) => {
      if (route.request().method() !== 'GET') {
        await route.fallback()
        return
      }
