 ...
 */
export function OrderDetailDialog({ orderUuid, open, onClose, onChanged }: OrderDetailDialogProps) {
  ...
  const [itemDrafts, setItemDrafts] = useState<EditableOrderItemDraft[]>([])
  ...
  useEffect(() => {
    ...
    Promise.resolve(orderService.getOrder(orderUuid))
      .then((order) => {
        if (cancelled) return
  ...
  const canEditItems = Boolean(selectedOrder) && !selectedOrder!.is_delivered && !selectedOrder!.is_paid && !selectedOrder!.cancelled_at

  const installmentsSum = useMemo(
    () => (selectedOrder?.installments ?? []).reduce((sum, installment) => sum + Number(installment.amount), 0),
    [selectedOrder],
  )

  const paidInstallmentsSum = useMemo(
    () => (selectedOrder?.installments ?? []).filter((installment) => installment.is_paid).reduce((sum, installment) => sum + Number(installment.amount), 0),
    [selectedOrder],
  )
  ...
  const installmentEditorMismatch = selectedOrder ? Math.abs(installmentEditorTotal - selectedOrder.total_amount) > 0.005 : false
  ...
  async function runAction(
    ...
    try {
      let updated: Order
      if (action === 'deliver') updated = await orderService.deliverOrder(selectedOrder.uuid)
      else if (action === 'pay') {
        const paidAt = paidAtDraft.trim() || undefined
        if (installmentUuid) {
          updated = await orderService.payInstallment(selectedOrder.uuid, installmentUuid, paidAt)
        } else {
          const amount = paidAmountDraft.trim() ? Number(paidAmountDraft) : undefined
          updated = await orderService.payOrder(selectedOrder.uuid, paidAt, amount)
        }
      } else if (action === 'approveCancellation') {
        updated = await orderService.approveOrderCancellationRequest(selectedOrder.uuid)
      } else if (action === 'rejectCancellation') {
        updated = await orderService.rejectOrderCancellationRequest(selectedOrder.uuid)
      } else {
        updated = await orderService.cancelOrder(selectedOrder.uuid, cancelReason.trim())
      }
  ...
  function openInstallmentEditor() {
    if (!selectedOrder) return
    const drafts: InstallmentDraftRow[] = (selectedOrder.installments ?? [])
  ...
  function addDraftRow() {
    setInstallmentDrafts((current) => {
      const paidNumbers = (selectedOrder?.installments ?? []).filter((installment) => installment.is_paid).map((installment) => installment.installment_number)
  ...
  async function saveInstallments() {
    ...
    try {
      const payload: OrderInstallmentsReallocatePayload = {
      ...
      await orderService.reallocateInstallments(selectedOrder.uuid, payload)
      await refetchSelectedOrder(selectedOrder.uuid)
      setInstallmentEditorOpen(false)
    } catch (err) {
  ...
  function openItemsEditor() {
    ...
    setHeaderStockLocationDraft(selectedOrder.stock_location?.uuid ?? '')
    setHeaderExpectedDeliveryDraft(selectedOrder.expected_delivery_date ? toDateOnly(selectedOrder.expected_delivery_date) : '')
  ...
  function updateItemDraftRow(id: string, patch: Partial<EditableOrderItemDraft>) {
  ...
  async function saveItemsEdits() {
    if (!selectedOrder || itemDrafts.length === 0 || itemsEditorHasIncompleteRow) return
    setItemsEditorError(null)
    setItemsFieldErrors({})
    setIsSavingItems(true)
    try {
      const payload: OrderUpdateItemsPayload = {
    ...
    <>
      <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
            ...
            <Stack spacing={2}>
              ...
              {selectedOrder.status === 'cancellation_requested' && (
                <Alert severity="warning" variant="outlined">
                  ...
                  {selectedOrder.cancellation_reason ? `: "${selectedOrder.cancellation_reason}"` : ', sem motivo informado.'}{' '}
                  Aprove ou rejeite abaixo.
                </Alert>
              )}
              {!itemsEditorOpen && (
                <>
                  <Typography><strong>Local de estoque:</strong> {selectedOrder.stock_location?.name ?? 'Padrão da empresa'}</Typography>
                  {selectedOrder.payment_method && (
                    <Typography>
                      <strong>Pagamento:</strong> {PAYMENT_METHOD_LABELS[selectedOrder.payment_method as PaymentMethod] ?? selectedOrder.payment_method}
                      {selectedOrder.needs_change &&
                        ` — troco para ${formatCurrency(selectedOrder.change_for_amount ?? 0)}`}
                    </Typography>
                  )}
                  <Typography><strong>Observações:</strong> {selectedOrder.notes ?? '—'}</Typography>
                </>
              )}
              ...
              {selectedOrder.status !== 'cancellation_requested' && !selectedOrder.cancelled_at && !selectedOrder.is_paid && (
          ...
          {!selectedOrder && isLoadingDetail && <Typography>Carregando...</Typography>}
        </DialogContent>
        <DialogActions sx={{ flexDirection: 'column', alignItems: 'stretch', gap: 1, px: 3, pb: 2 }}>
          ...
          {selectedOrder && selectedOrder.status !== 'cancellation_requested' && !selectedOrder.is_delivered && !selectedOrder.cancelled_at && (
            <Button startIcon={<LocalShippingOutlinedIcon />} disabled={isSubmittingAction} onClick={() => void runAction('deliver')} sx={{ minHeight: 44 }}>
              Entregar
            </Button>
          )}
          {selectedOrder && selectedOrder.status !== 'cancellation_requested' && !selectedOrder.is_installment && !selectedOrder.is_paid && !selectedOrder.cancelled_at && (
            <Button startIcon={<PaidOutlinedIcon />} disabled={isSubmittingAction} onClick={() => void runAction('pay')} sx={{ minHeight: 44 }}>
              Marcar como pago
            </Button>
          )}
          {selectedOrder && selectedOrder.status !== 'cancellation_requested' && !selectedOrder.cancelled_at && !selectedOrder.is_paid && (
            <Button
              color="error"
              startIcon={<CancelOutlinedIcon />}
