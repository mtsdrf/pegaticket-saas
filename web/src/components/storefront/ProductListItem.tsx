import AddIcon from '@mui/icons-material/Add'
import FavoriteIcon from '@mui/icons-material/Favorite'
import FavoriteBorderOutlinedIcon from '@mui/icons-material/FavoriteBorderOutlined'
import RemoveIcon from '@mui/icons-material/Remove'
import StorefrontOutlinedIcon from '@mui/icons-material/StorefrontOutlined'
import { Box, Button, Chip, IconButton, Paper, Stack, Typography } from '@mui/material'
import { ProductCardBadges } from './ProductCardBadges'
import { useProductCardState } from '../../hooks/useProductCardState'
import { ELEVATED_SURFACE_SX, SOFT_PANEL_SX } from '../../styles/surfaces'
import type { StorefrontProduct } from '../../types/storefront'
import { formatCurrency } from '../../utils/format'

interface ProductListItemProps {
  product: StorefrontProduct
  onToggleFavorite: (product: StorefrontProduct) => void
  onConfigureOptions: (product: StorefrontProduct) => void
}

/**
 * Item de produto da loja pública — lista com imagem à direita (nome/descrição
 * à esquerda, foto menor à direita, preço abaixo). Layout padrão
 * (`tenant_settings.catalog_layout === 'list'`); a alternativa é
 * `ProductGridCard` (foto grande). Os dois compartilham estado/lógica via
 * `useProductCardState`, só a estrutura visual difere.
 */
export function ProductListItem({ product, onToggleFavorite, onConfigureOptions }: ProductListItemProps) {
  const { quantity, hasOptions, discountLabel, showWholesaleNote, addItem, updateQuantity } =
    useProductCardState(product)

  return (
    <Paper
      elevation={0}
      sx={{
        ...ELEVATED_SURFACE_SX,
        p: 1.5,
        display: 'flex',
        alignItems: 'stretch',
        gap: 1.5,
        opacity: product.is_available ? 1 : 0.6,
        transition: 'box-shadow 0.15s ease',
        '&:hover': { boxShadow: 'var(--mk-shadow-md)' },
      }}
    >
      <Box sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        <ProductCardBadges badges={product.badges} />

        <Typography sx={{ fontSize: 14.5, fontWeight: 700, lineHeight: 1.3, wordBreak: 'break-word' }}>
          {product.name}
        </Typography>

        {product.description && (
          <Typography
            sx={{
              fontSize: 12.5,
              color: 'var(--mk-muted)',
              mt: 0.25,
              display: '-webkit-box',
              WebkitLineClamp: 2,
              WebkitBoxOrient: 'vertical',
              overflow: 'hidden',
            }}
          >
            {product.description}
          </Typography>
        )}

        <Box sx={{ flex: 1 }} />

        <Stack spacing={0.5} sx={{ mt: 1, alignItems: 'center', textAlign: 'center' }}>
          {product.promo_price !== null ? (
            <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
              <Typography sx={{ fontSize: 12, color: 'var(--mk-muted)', textDecoration: 'line-through' }}>
                {formatCurrency(product.price)}
              </Typography>
              <Typography sx={{ fontSize: 16, fontWeight: 700, color: 'var(--mk-primary)' }}>
                {formatCurrency(product.promo_price)}
              </Typography>
              {discountLabel && (
                <Chip
                  label={discountLabel}
                  size="small"
                  sx={{ height: 18, fontSize: 10, fontWeight: 700, color: '#FFFFFF', bgcolor: 'var(--mk-danger)' }}
                />
              )}
            </Stack>
          ) : (
            <Typography sx={{ fontSize: 16, fontWeight: 700, color: 'var(--mk-primary)' }}>
              {formatCurrency(product.price)}
            </Typography>
          )}

          {showWholesaleNote && (
            <Typography sx={{ fontSize: 11.5, fontWeight: 600, color: 'var(--mk-success, #1b7a3d)', lineHeight: 1.25 }}>
              A partir de {String(product.wholesale_min_quantity).replace('.', ',')} un.:{' '}
              {formatCurrency(product.wholesale_price!)} cada
            </Typography>
          )}

          {!product.is_available ? (
            <Typography sx={{ fontSize: 12.5, fontWeight: 600, color: 'var(--mk-muted)' }}>
              Indisponível no momento
            </Typography>
          ) : hasOptions ? (
            <Stack spacing={0.5} sx={{ alignItems: 'flex-start' }}>
              <Button
                variant="outlined"
                size="small"
                startIcon={<AddIcon />}
                onClick={() => onConfigureOptions(product)}
                sx={{ minHeight: 44 }}
              >
                Escolher opcionais
              </Button>
              {quantity > 0 && (
                <Typography sx={{ fontSize: 11.5, color: 'var(--mk-muted)' }}>
                  {quantity} item(ns) deste produto no carrinho
                </Typography>
              )}
            </Stack>
          ) : quantity === 0 ? (
            <Button
              variant="outlined"
              size="small"
              startIcon={<AddIcon />}
              onClick={() => addItem(product)}
              fullWidth
              sx={{ minHeight: 44 }}
            >
              Adicionar
            </Button>
          ) : (
            <Stack direction="row" spacing={0.5} sx={{ alignItems: 'center' }}>
              <IconButton
                size="small"
                aria-label={`Diminuir quantidade de ${product.name}`}
                onClick={() => updateQuantity(product.uuid, quantity - 1)}
                sx={{ minWidth: 44, minHeight: 44, ...SOFT_PANEL_SX }}
              >
                <RemoveIcon fontSize="small" />
              </IconButton>
              <Typography sx={{ fontWeight: 700, minWidth: 24, textAlign: 'center' }}>{quantity}</Typography>
              <IconButton
                size="small"
                aria-label={`Aumentar quantidade de ${product.name}`}
                onClick={() => updateQuantity(product.uuid, quantity + 1)}
                sx={{ minWidth: 44, minHeight: 44, ...SOFT_PANEL_SX }}
              >
                <AddIcon fontSize="small" />
              </IconButton>
            </Stack>
          )}
        </Stack>
      </Box>

      <Box
        sx={{
          position: 'relative',
          width: { xs: 88, sm: 104 },
          flexShrink: 0,
          ...SOFT_PANEL_SX,
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          overflow: 'hidden',
          borderRadius: 'var(--mk-radius-md)',
        }}
      >
        {product.image_url ? (
          <Box
            component="img"
            src={product.image_url}
            alt={product.name}
            sx={{ width: '100%', height: '100%', objectFit: 'cover' }}
          />
        ) : (
          <StorefrontOutlinedIcon sx={{ fontSize: 30, color: 'var(--mk-muted)' }} />
        )}

        <IconButton
          onClick={(event) => {
            event.stopPropagation()
            onToggleFavorite(product)
          }}
          aria-label={product.is_favorited ? `Remover ${product.name} dos favoritos` : `Favoritar ${product.name}`}
          size="small"
          sx={{
            position: 'absolute',
            top: 0,
            right: 0,
            width: 40,
            height: 40,
            bgcolor: 'color-mix(in srgb, var(--mk-surface) 85%, transparent)',
            '&:hover': { background: 'var(--mk-surface)' },
          }}
        >
          {product.is_favorited ? (
            <FavoriteIcon sx={{ fontSize: 17, color: 'var(--mk-primary)' }} />
          ) : (
            <FavoriteBorderOutlinedIcon sx={{ fontSize: 17, color: 'var(--mk-muted)' }} />
          )}
        </IconButton>
      </Box>
    </Paper>
  )
}
