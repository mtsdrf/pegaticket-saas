...
const TRAINING_MODULES: TrainingModule[] = [
  ...
  {
    id: 'estoque',
    name: 'Estoque e disponibilidade',
    summary: 'Mostra como o saldo nasce, como a reserva funciona e por que pedidos dependem do estoque correto.',
    status: 'implemented',
    route: '/estoque/saldos',
    audience: ['Estoque', 'Operação', 'Gestão'],
    functionality: 'stock',
    requirement: ACCESS.stockRead,
    whoUses: 'Estoquista, gestor e operação que controla disponibilidade.',
    dependencies: ['catalogo'],
    connectsTo: ['pedidos', 'pdv', 'balcao', 'delivery'],
    teaches: [
      'Como reservas e baixas evitam venda acima do saldo',
      'Quando usar ajuste, perda, bloqueio ou transferência',
      'Como ler o saldo para entender se o pedido pode seguir',
    ],
    risks: [
      'Movimento incorreto distorce saldo e margens',
      'Estoque sem local padrão bloqueia parte da operação de pedidos',
    ],
    operations: [
      {
        title: 'Registrar entrada inicial',
        purpose: 'Dar saldo operacional aos produtos antes da venda.',
        when: 'No início da implantação ou em reposição.',
        actor: 'Estoquista ou gestor',
        route: '/estoque/movimentos/nova',
        permissionLabel: 'stock:create',
        requirements: ['Local de estoque criado', 'Produto ativo'],
        effects: ['Atualiza saldo disponível e libera operação dependente de estoque'],
        commonErrors: ['Produto sem local correto', 'Lançar ajuste quando a intenção era entrada'],
        goodPractices: ['Separar tipos de movimento pelo evento real', 'Conferir quantidade e custo antes de salvar'],
      },
    ],
  },
  {
    id: 'clientes',
    name: 'Clientes e relacionamento',
    summary: 'Ensina a preparar base de clientes e os dados que sustentam pedidos, cobrança e entrega.',
    status: 'implemented',
    route: '/clientes',
    audience: ['Comercial', 'Atendimento', 'Financeiro'],
    functionality: 'clients',
    requirement: ACCESS.clientsRead,
    whoUses: 'Atendimento, comercial e financeiro.',
    dependencies: ['empresa'],
    connectsTo: ['pedidos', 'financeiro', 'loja', 'relatorios'],
    teaches: [
      'Quando o cliente precisa existir antes do pedido',
      'Por que endereço e telefone influenciam entrega e cobrança',
      'Como categorias ajudam preço e segmentação',
    ],
    risks: [
    ...
    operations: [
      {
        title: 'Cadastrar cliente pronto para pedido',
        purpose: 'Criar cliente com dados suficientes para venda e acompanhamento.',
        when: 'Antes da primeira venda recorrente ou da cobrança assistida.',
        actor: 'Atendimento ou comercial',
        route: '/clientes/novo',
        permissionLabel: 'clients:create',
        requirements: ['Endereço disponível quando a operação usa entrega', 'Categoria definida quando houver preço por categoria'],
        effects: ['Cliente passa a ser elegível para pedidos, relatórios e cobrança'],
        commonErrors: ['Telefone principal ausente', 'Categoria comercial não sincronizada'],
        goodPractices: ['Preencher contatos úteis e endereço real logo no primeiro cadastro'],
      },
    ],
  },
  {
  ...
  {
    id: 'financeiro',
    name: 'Financeiro e recebíveis',
    summary: 'Ensina como o sistema acompanha pagamento, atraso, carteira e conciliação.',
    status: 'implemented',
    route: '/relatorios/recebiveis',
    audience: ['Financeiro', 'Gestão'],
    functionality: 'reports',
    requirement: ACCESS.reportsRead,
    whoUses: 'Financeiro, proprietários e gestão.',
    dependencies: ['pedidos'],
    connectsTo: ['analytics', 'assinatura'],
    teaches: [
      'Como ler carteira em aberto, aging e projeção',
      'Quando pagar pedido, parcela ou conciliar divergência',
      'Como usar histórico de interação para cobrança',
    ],
    risks: [
      'Baixa indevida altera indicadores e conciliação',
      'Cobrança sem histórico prejudica continuidade operacional',
    ],
    operations: [
      {
        title: 'Trabalhar a fila de recebíveis',
        purpose: 'Acompanhar títulos em aberto e agir sobre atraso.',
        when: 'Na rotina diária ou semanal do financeiro.',
        actor: 'Financeiro',
        route: '/relatorios/recebiveis',
        permissionLabel: 'reports:read',
        requirements: ['Pedidos registrados', 'Fluxo de pagamento já usado na operação'],
        effects: ['Expõe aging, cobrança assistida e histórico de interação'],
        commonErrors: ['Interpretar pedido cancelado como dívida ativa', 'Cobrar sem validar status real do pedido'],
        goodPractices: ['Usar resumo + histórico para decidir a próxima ação de cobrança'],
      },
    ],
  },
  {
  ...
  },
  {
    id: 'pdv',
    name: 'PDV e balcão',
    summary: 'Ensina o fluxo rápido de venda presencial, caixa e operação de balcão/mesa.',
    status: 'implemented',
    route: '/pdv',
    audience: ['Operação presencial', 'Caixa', 'Restaurante'],
    functionality: 'pdv',
    requirement: ACCESS.pdvRead,
    whoUses: 'Operadores de caixa, atendimento de balcão e cozinha/bar.',
    dependencies: ['catalogo', 'estoque'],
    connectsTo: ['pedidos', 'financeiro'],
    teaches: [
      'Diferença entre PDV, balcão e pedido tradicional',
      'Como a sessão de caixa e o PIN do operador protegem a operação',
      'Quando mesa/comanda viram pedido financeiro consolidado',
    ],
    risks: [
      'Abrir operação presencial sem sessão de caixa',
      'Misturar fluxo de mesa com fluxo de pedido comum sem conferência',
    ],
    operations: [
      {
        title: 'Iniciar operação de PDV',
        purpose: 'Abrir a jornada presencial com caixa controlado.',
        when: 'No início do turno de venda presencial.',
        actor: 'Operador ou gestor de caixa',
        route: '/pdv',
        permissionLabel: 'pdv:read/open',
        requirements: ['Permissão PDV', 'Estrutura de produtos pronta'],
        effects: ['Abre contexto de sessão para venda, recibo e fechamento'],
        commonErrors: ['Entrar no PDV sem sessão ativa', 'Operador sem PIN configurado em cenários que exigem rastreio'],
        goodPractices: ['Conferir abertura e fechamento de sessão por turno'],
      },
    ],
  },
]
...
const TRAINING_TRACKS: TrainingTrack[] = [
  ...
  {
    id: 'financeiro',
    title: 'Cobrança, recebíveis e visão de caixa',
    audience: 'finance',
    description: 'Treinamento focado em carteira, baixa, conciliação e leitura dos indicadores financeiros.',
    modules: ['pedidos', 'financeiro', 'empresa'],
    outcome: 'Financeiro lê a carteira com contexto e reduz retrabalho em cobrança.',
  },
  {
    id: 'fiscal-contador',
...
const TRAINING_QUIZZES: Record<string, TrainingQuiz> = {
  ...
  estoque: {
    question: 'Se a intenção foi registrar reposição de mercadoria, qual é o tipo mental de ação correta?',
    options: ['Bloqueio', 'Entrada', 'Perda'],
    correctIndex: 1,
    explanation: 'Entrada é a ação que nasce para aumentar disponibilidade; ajuste e perda têm outros objetivos operacionais.',
  },
  clientes: {
    question: 'Por que o endereço do cliente importa antes mesmo da entrega acontecer?',
    options: [
      'Porque não importa para nenhuma outra área',
      'Porque influencia taxa, rota, rastreio e leitura do histórico',
      'Porque substitui a necessidade do pedido',
    ],
    correctIndex: 1,
    explanation: 'No Maskats, endereço impacta mais do que entrega: ele ajuda taxa, logística, rastreio e leitura operacional.',
  },
  pedidos: {
  ...
  financeiro: {
    question: 'O que ajuda mais a cobrança assistida no Maskats?',
    options: [
      'Cobrar sem registrar contexto',
      'Usar resumo de recebíveis e histórico de interação',
      'Ignorar status do pedido e cobrar tudo igual',
    ],
    correctIndex: 1,
    explanation: 'O valor do fluxo financeiro aumenta quando a equipe registra contexto e usa a carteira com histórico.',
  },
  fiscal: {
  ...
  pdv: {
    question: 'Antes de vender em fluxo presencial, qual validação é a mais importante?',
    options: [
      'Sessão de caixa pronta e contexto operacional válido',
      'Somente abrir o recibo',
      'Ir direto para o fechamento do caixa',
    ],
    correctIndex: 0,
    explanation: 'PDV e balcão dependem de sessão e contexto de operação para manter rastreabilidade e fechamento corretos.',
  },
}
