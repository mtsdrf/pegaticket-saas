...
export function EventCategoryFormPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (uuid) {
        await eventCategoryService.updateEventCategory(uuid, payload)
      } else {
        await eventCategoryService.createEventCategory(payload)
      }
      navigate('/eventos/categorias')
    } catch (err) {
  ...
  return (
    <SchemaFormPage
      ...
      backTo="/eventos/categorias"
      title={isEditMode ? 'Editar categoria' : 'Nova categoria'}
      subtitle={isEditMode ? 'Atualize os dados da categoria.' : 'Cadastre uma nova categoria de evento.'}
      breadcrumbs={[
        { label: 'Eventos', to: '/eventos' },
        { label: 'Categorias', to: '/eventos/categorias' },
