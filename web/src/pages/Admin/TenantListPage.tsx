...
export function TenantListPage() {
  ...
  const columns = useMemo<ServerGridColumn<Tenant>[]>(() => [
    ...
    { field: 'trial_ends_at', headerName: 'Fim do trial', filterType: 'text', sortable: false, cellRenderer: (row) => row.trial_ends_at ? formatDateBR(row.trial_ends_at) : '—', exportValue: (row) => row.trial_ends_at ? formatDateBR(row.trial_ends_at) : '' },
  ...
  return (
    <>
      <CrudListPage title="Empresas" subtitle="Gerencie as empresas da plataforma." createLabel="Nova empresa" canCreate={can(ACCESS.adminTenantsCreate)} onCreate={() => navigate('/admin/tenants/novo')} error={null} onRetry={() => undefined} isLoading={isLoading} isEmpty={false}>
        <ServerDataGrid columns={columns} fetchPage={fetchPage} rowIdField="uuid" onGridReady={(api) => { gridApiRef.current = api }} emptyState={{ icon: <ApartmentOutlinedIcon sx={{ fontSize: 40, color: 'var(--pt-muted)' }} />, title: 'Nenhuma empresa cadastrada', description: 'Cadastre empresas para separar a operação multiempresa.', action: can(ACCESS.adminTenantsCreate) ? <Button variant="contained" onClick={() => navigate('/admin/tenants/novo')}>Cadastrar primeira empresa</Button> : undefined }} />
      </CrudListPage>
