...
import { rateOrder, type OrderRatingResult } from '../../services/orderRatingService'
...
function StatusBanner({ order }: { order: OrderTracking }) {
...
function InstallmentsList({ installments }: { installments: OrderTracking['installments'] }) {
  return (
    <Stack spacing={1.25}>
 ...
 */
function LinkOrdersSection({ orderUuid }: { orderUuid: string }) {
  ...
  async function handleClick() {
    ...
    if (!isAuthenticated) {
      navigate(`/portal/entrar?vincular=${encodeURIComponent(orderUuid)}`)
      return
    }

    setIsLinking(true)
    try {
      await createPortalLink({ order_uuid: orderUuid })
 ...
 */
function OrderRatingSection({ orderUuid }: { orderUuid: string }) {
  const [ratingValue, setRatingValue] = useState<number | null>(null)
  const [comment, setComment] = useState('')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [errorMessage, setErrorMessage] = useState<string | null>(null)
  const [submitted, setSubmitted] = useState<OrderRatingResult | null>(null)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await rateOrder(orderUuid, { rating: ratingValue, comment: comment.trim() || undefined })
      setSubmitted(result)
    } catch (error) {
      ...
      <Box sx={{ ...PAGE_CONTAINER_SX, maxWidth: 480 }}>
        ...
        {!isLoading && order && (
          <Stack spacing={2.5}>
            ...
            {order.is_delivered && isPortalAuthenticated && <OrderRatingSection orderUuid={order.uuid} />}

            <LinkOrdersSection orderUuid={order.uuid} />
          </Stack>
        )}
